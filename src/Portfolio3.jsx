...
const skyGlowHover = {
  whileHover: {
    boxShadow: "0 0 10px rgba(96, 165, 250, 0.4), 0 0 20px rgba(96, 165, 250, 0.2)"
  }
};
const whiteGlowHover = {
  whileHover: {
...
const cardGlowHover = {
  whileHover: {
    boxShadow: "0 0 10px rgba(96, 165, 250, 0.4), 0 0 20px rgba(255, 255, 255, 0.2)",
    scale: 1.02
  }
};
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{
          background: 'radial-gradient(circle at top right, #60A5FA40, transparent 50%), radial-gradient(circle at bottom left, #fff4, transparent 50%)',
        }}
      />
    </motion.div>
  );
...
const GlowButton = ({ children, className = "", ...props }) => (
  <motion.button
    {...props}
    className={`px-6 py-3 rounded-full font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:ring-offset-2 focus-visible:ring-offset-black transition-all duration-300 ${className}`}
  >
    {children}
  </motion.button>
);
...
const Pill = ({ children, className = "" }) => (
  <span className={`inline-flex items-center px-3 py-1 text-xs font-medium rounded-full ring-1 ring-inset ring-blue-400/30 bg-blue-400/10 text-blue-300 ${className}`}>
    {children}
  </span>
);
    ...
    <div className="bg-black text-white antialiased font-sans relative overflow-x-hidden min-h-screen">
      ...
      >
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <a href="#" className="flex items-center space-x-2 text-white font-bold text-lg group focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 rounded-lg p-2 transition-all">
            <Star className="text-blue-400 group-hover:scale-110 transition-transform" size={24} />
            ...
            {navLinks.map((link) => (
              <li key={link.name}>
                <button
                  ...
                  className={`text-sm font-medium transition-colors hover:text-blue-400 focus:outline-none focus-visible:text-blue-400 ${activeSection === link.name.toLowerCase() ? 'text-blue-400' : 'text-gray-400'}`}
                >
                  {link.name}
                </button>
              </li>
      ...
      <main className="container mx-auto px-4 pt-28">
        ...
        <motion.section ref={sections.about} id="about" className="min-h-screen grid grid-cols-1 md:grid-cols-2 gap-12 items-center mb-24" {...fadeIn}>
          <div className="space-y-6">
            <motion.p className="text-blue-400 text-lg font-medium tracking-wide" {...fadeInUp}>👋 I am a:</motion.p>
            ...
            <motion.div className="flex flex-col sm:flex-row gap-4 pt-4" {...fadeInUp}>
              <GlowButton as="a" href={DUMMY.contacts.linkedin} target="_blank" rel="noopener noreferrer" className="bg-blue-400 text-black hover:bg-blue-500">
              ...
              <GlowButton as="a" href={`mailto:${DUMMY.contacts.email}`} className="border border-blue-400 text-blue-400 hover:bg-blue-400/10 hover:text-blue-300">
          ...
          <motion.div className="grid grid-cols-1 sm:grid-cols-3 gap-6" {...fadeInUp}>
            {Object.entries(DUMMY.stats).map(([key, value]) => (
              <GlassCard key={key} className="flex flex-col items-center text-center p-6 hover:scale-105 transition-transform duration-300">
                <motion.span className="text-5xl font-bold text-blue-400" initial={{ scale: 0.8 }} animate={{ scale: 1 }} transition={{ delay: 0.5, type: "spring", stiffness: 300 }}>
                  {value}
                </motion.span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {DUMMY.codes.map((snippet, index) => (
              <GlassCard key={index} className="flex flex-col p-4 md:p-6">
                ...
                <pre className="overflow-auto bg-black border border-blue-400/20 rounded-xl p-4 text-sm text-gray-300 font-mono max-h-96">
        ...
        <section ref={sections.certifications} id="certifications" className="mb-24">
          ...
          <div className="flex justify-center mt-8 space-x-4">
            <button
              ...
              className="p-3 rounded-full bg-gray-800 text-gray-400 hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400"
                    ...
                    <motion.div key={project.id} {...cardGlowHover} className="flex flex-col md:flex-row bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden shadow-xl">
                      ...
                      <div className="p-6 md:p-8 flex-grow space-y-4">
                        ...
                        <div className="flex gap-4 pt-2">
                          <GlowButton as="a" href={project.demo} target="_blank" rel="noopener noreferrer" className="bg-blue-400 text-black hover:bg-blue-500">
                            Demo
                          </GlowButton>
        ...
        <section ref={sections.awards} id="awards" className="mb-24">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {DUMMY.awards.map((award, index) => (
              <motion.div key={index} {...hoverLift} {...skyGlowHover} {...fadeInUp} className="rounded-2xl bg-gray-900 p-6 border border-gray-800">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            <GlassCard className="p-6 md:p-8">
              ...
              <form onSubmit={handleFormSubmit} className="space-y-4">
                <div>
                  ...
                  <input type="text" id="name" name="name" placeholder="Name" required className="w-full p-3 bg-gray-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-400 transition-all" />
                </div>
                <div>
                  <label htmlFor="email" className="sr-only">Email</label>
                  <input type="email" id="email" name="email" placeholder="Email" required className="w-full p-3 bg-gray-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-400 transition-all" />
                </div>
                <div>
                  <label htmlFor="message" className="sr-only">Message</label>
                  <textarea id="message" name="message" rows="5" placeholder="Message" required className="w-full p-3 bg-gray-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-400 transition-all" />
                </div>
            ...
            <GlassCard className="p-6 md:p-8">
              ...
              <div className="flex flex-wrap gap-4">
                ...
                <GlowButton as="a" href={DUMMY.contacts.github} target="_blank" rel="noopener noreferrer" className="border border-white/20 text-white hover:bg-white/10">
                  <Github size={20} className="inline mr-2" /> GitHub
                </GlowButton>
                <GlowButton as="a" href={`mailto:${DUMMY.contacts.email}`} className="border border-white/20 text-white hover:bg-white/10">
                  <Mail size={20} className="inline mr-2" /> Email
                </GlowButton>
                <GlowButton as="a" href={DUMMY.contacts.leetcode} target="_blank" rel="noopener noreferrer" className="border border-white/20 text-white hover:bg-white/10">
                  LeetCode
                </GlowButton>
              </div>
            </GlassCard>
      ...
      <div className="flex justify-center md:justify-start space-x-2 sm:space-x-4 mb-8 border-b border-gray-700">
        {tabs.map((tab) => (
          <button
            ...
            className={`px-4 py-2 text-sm sm:text-base font-medium rounded-t-lg transition-colors duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 ${
        ...
        <GlassCard>
          <div className="flex items-center space-x-4">
            ...
            <div className="flex-grow">
              ...
              <p className="text-gray-400 text-sm">{participation.org} • <span className="text-blue-400">{participation.result}</span></p>
            </div>
